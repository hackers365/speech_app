...
class Recognizer(AudioSource):
    ...
    def samples_to_amr(self, source, frame_data):
        ...
        file_obj = open('/tmp/1.wav', 'w')
        file_obj.write(wav_data)
        file_obj.close()

        amr_converter='/usr/bin/avconv'

        process = subprocess.Popen("\"%s\" -i pipe:0 -ar 8000 -acodec libopencore_amrnb -f amr -" % amr_converter, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
        amr_data, stderr = process.communicate(wav_data)

        file_obj = open('/tmp/1.amr', 'w')
        file_obj.write(amr_data)
        file_obj.close()
    ...
    def listen(self, source, timeout = None):
        ...
        while True:
            ...
            print('pause_count:' + str(pause_count))
        ...
        return AudioData(8000, self.samples_to_amr(source, frame_data))
        #~ return AudioData(source.RATE, self.samples_to_flac(source, frame_data))
    ...
    def recognize(self, audio_data):
        ...
        try:
            response = urlopen(self.request, timeout=5)
        except URLError:
            raise IndexError("No internet connection available to transfer audio data")
        except:
            raise KeyError("Server wouldn't respond (invalid key or quota has been maxed out)")
